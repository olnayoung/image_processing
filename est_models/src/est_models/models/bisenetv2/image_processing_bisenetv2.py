...
class BiseNetV2ImageProcessor(BaseImageProcessor):
    ...
    def image_to_tensor(self, image):
        ...
        return self.transform(image).unsqueeze(dim=0)

    def tensor_to_image(self, tensor, label_type=None):
        warnings.warn(
            "tensor_to_image is deprecated and will be removed in a future version. Please use post_process_semantic_segmentation."
        )

        return self.post_process_semantic_segmentation(tensor, label_type)

    def post_process_semantic_segmentation(
        self, outputs, target_sizes: List[Tuple] = None
    ):
        """
        Converts the output of [`BiSeNetV2ForSemanticSegmentation`] into semantic segmentation maps. Only supports
        PyTorch.

        Args:
            outputs ([`BiSeNetV2ForSemanticSegmentation`]):
