...
class PPMattingV2ImageProcessor(BaseImageProcessor):
    ...
    def post_process_human_matting(self, logits, target_sizes: List[Tuple] = None):
        ...
        if target_sizes is not None:
            ...
            for idx in range(len(logits)):
                ...
                binary_map = resized_logits.squeeze(0).permute(1, 2, 0).squeeze(dim=-1).numpy()
                alpha_masks.append(binary_map)
        else:
            alpha_masks = [
                logits[i].permute(1, 2, 0).squeeze(dim=-1).numpy()
