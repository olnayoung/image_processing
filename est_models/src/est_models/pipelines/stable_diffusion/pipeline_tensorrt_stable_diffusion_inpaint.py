...
class TensorRTStableDiffusionInpaintPipeline(StableDiffusionInpaintPipeline):
    ...
    def __loadModels(self):
        ...
        if "unet" in self.stages:
            self.models["unet"] = make_UNet(
                self.unet, **models_args, unet_dim=self.unet.config.in_channels
            )
    ...
    def to(
        ...
        if not self.cached_folder:
            raise Exception("Check if [pipe.set_cached_folder(<weight_path>)] in your code")
    ...
    def initialize_timesteps(self, num_inference_steps, strength):
