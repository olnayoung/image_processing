...
class StableDiffusionWebuiInpaintPipeline(StableDiffusionInpaintPipeline):
    @torch.no_grad()
    def __call__(
        self,
        ...
        **kwargs,
        ...
        if mask_image is not None:
            if isinstance(mask_image, list):
                mask_image = [mask.convert("L") for mask in mask_image]
            else:
        ...
        if not output_type == "latent":
            ...
            # 쌍커풀 만들 때 nsfw라고 생성되지 않는 것을 막기 위함
        ...
        image = [apply_overlay(x, paste_to, overlay_images[0]) for x in image]
