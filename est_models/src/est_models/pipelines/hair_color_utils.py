...
def normal(img_a, img_b, opacity):
    ...
    img_a_alp = (
        np.expand_dims(img_a[:, :, 3], 2) * opacity
    )  # alpha of a, prepared for broadcasting
    img_b_alp = np.expand_dims(
        img_b[:, :, 3], 2
    )  # alpha of b, prepared for broadcasting

    # Blend images
    c_out = (
        img_a[:, :, :3] * img_a_alp + img_b[:, :, :3] * img_b_alp * (1 - img_a_alp)
...
def soft_light(img_in, img_layer, opacity):
    ...
    assert (
        img_in.dtype.kind == "f"
    ), "Input variable img_in should be of numpy.float type."
    assert (
        img_layer.dtype.kind == "f"
    ), "Input variable img_layer should be of numpy.float type."
    assert img_in.shape[2] == 4, "Input variable img_in should be of shape [:, :,4]."
    assert (
        img_layer.shape[2] == 4
    ), "Input variable img_layer should be of shape [:, :,4]."
    ...
    comp = (1.0 - img_in[:, :, :3]) * img_in[:, :, :3] * img_layer[:, :, :3] + img_in[
        :, :, :3
...
def hard_light(img_in, img_layer, opacity):
    ...
    comp = np.greater(img_layer[:, :, :3], 0.5) * np.minimum(
        1.0 - ((1.0 - img_in[:, :, :3]) * (1.0 - (img_layer[:, :, :3] - 0.5) * 2.0)),
        1.0,
...
def overlay(img_in, img_layer, opacity):
    ...
    comp = np.less(img_in[:, :, :3], 0.5) * (
        2 * img_in[:, :, :3] * img_layer[:, :, :3]
...
class HairColorizer:
    def __call__(
        ...
        background_img = cv2.cvtColor(background_img, cv2.COLOR_RGB2RGBA).astype(float)

        # make color RGBA mask
        mask_color = np.array(mask) > 0
        R, G, B, A = [np.zeros_like(mask_color).astype(np.uint8) for _ in range(4)]
        R[mask_color], G[mask_color], B[mask_color] = color
        A[mask_color] = 255
        color_mask = np.dstack([R, G, B, A]).astype(np.float32)

        # blurry mask
        kernel = np.ones((5, 5), np.float32) / 25
        mask_color_float = cv2.filter2D(color_mask, -1, kernel)

        blended_img_float = soft_light(background_img, mask_color_float, opacity=0.7)
        blended_img_uint8 = blended_img_float.astype(np.uint8)
        return Image.fromarray(blended_img_uint8)
