...
class CLIPTextCustomEmbedder(object):
    def __init__(self, tokenizer, text_encoder, device, clip_stop_at_last_layers=1):
    ...
    def process_tokens(self, remade_batch_tokens, batch_multipliers):
        ...
        # print(tokens.shape)
        # print(tokens)
        outputs = self.text_encoder(input_ids=tokens, output_hidden_states=True)
...
def text_embeddings_equal_len(text_embedder, prompt, negative_prompt):
    ...
    if cond_len == uncond_len:
        ...
        if cond_len > uncond_len:
            ...
            return cond_embeddings, torch.cat(
                [uncond_embeddings] + [text_embedder("")] * n, dim=1
            )
        else:
            n = (uncond_len - cond_len) // 77
            return (
                torch.cat([cond_embeddings] + [text_embedder("")] * n, dim=1),
                uncond_embeddings,
            )


def text_embeddings(pipe, prompt, negative_prompt, clip_stop_at_last_layers=1):
    text_embedder = CLIPTextCustomEmbedder(
        tokenizer=pipe.tokenizer,
        text_encoder=pipe.text_encoder,
        device=pipe.text_encoder.device,
        clip_stop_at_last_layers=clip_stop_at_last_layers,
    )
    cond_embeddings, uncond_embeddings = text_embeddings_equal_len(
        text_embedder, prompt, negative_prompt
    )
    return cond_embeddings, uncond_embeddings
